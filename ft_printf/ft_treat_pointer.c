...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/10 15:03:08 by mde-gius          #+#    #+#             */
/*   Updated: 2021/02/10 15:03:10 by mde-gius         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_treat_pointer(unsigned long long ull, t_flags flags)
{
	...
	char_count = 0;
	if (ull == 0 && flags.dot == 0 && flags.width != 0)
	{
		char_count += ft_treat_width2(flags.width);
		return (char_count += ft_putstrprec("0x", 2));
	}
