...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/10 15:02:39 by mde-gius          #+#    #+#             */
/*   Updated: 2021/02/10 15:02:44 by mde-gius         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_treat_int(int i, t_flags flags)
{
	...
	char_count = 0;
	if (flags.zero == 1 && flags.minus == 1 && flags.width && i < 0)
		return (ft_treat_s_case(i, flags.width));
	if (flags.dot == 0 && i == 0)
	{
		char_count += ft_treat_width(flags.width, 0, 0);
		return (char_count);
	}
	if ((save_i = i) < 0 && (flags.dot >= 0 || flags.zero == 1))
	{
